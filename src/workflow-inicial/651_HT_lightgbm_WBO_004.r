...
#' Utilización del método Walk Forward Validation con caída exponencial:
#' - De base "natural": e^x
#' - De base "a": a^x ,donde: "0 < a < 1"
...
PARAM <- list()
PARAM$experimento <- "HT6510_WBO_4"

PARAM$exp_input <- "TS6410"

# Parametros para WFV con decay exponencial
ROUND_DIGITS = 6
W_DECAY_TYPE = "DIS" #  valores:"EXP", "POW" , "DIS"
...
EstimarGanancia_lightgbm <- function(x) {
  ...
  param_completo$early_stopping_rounds <- as.integer(400 + 4 / param_completo$learning_rate)
  ...
  dtrain <- lgb.Dataset(
    ...
    weight = weights_row + dataset[ fold_train == 1, 
