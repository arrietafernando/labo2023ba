#'
#' Ref: https://www.r-bloggers.com/2018/07/machine-learning-results-in-r-one-plot-to-rule-them-all/
#'
...
mplot_density <-
  function(tag,
    ...
    if (length(unique(tag)) != 2) {
      stop(
        "This function is for binary models. You should only have 2 unique values for the tag value!"
      )
    }
    ...
    p3 <- ggplot(out) + theme_minimal() +
      geom_line(aes(
        x = score * 100,
        y = 100 * (1 - ..y..),
        color = as.character(tag)
      ),
      stat = 'ecdf',
      size = 1) +
      geom_line(
        aes(x = score * 100, y = 100 * (1 - ..y..)),
        stat = 'ecdf',
        size = 0.5,
        colour = "black",
        linetype = "dotted"
...
mplot_roc <-
  function(tag,
    ...
    coords <- data.frame(x = rev(roc$specificities),
    ...
    p <- ggplot(coords, aes(x = x, y = y)) +
      ...
      linetype = "dotted") +
      scale_x_reverse(
        name = "% Specificity [False Positive Rate]",
        limits = c(1, 0),
